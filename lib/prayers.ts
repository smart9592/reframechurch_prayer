...
const HOUR_MAP: Record<string, HourDoc> = {
  prime:    hourPrime    as HourDoc,
  terce:    hourTerce    as HourDoc,
  sext:     hourSext     as HourDoc,
  none:     hourNone     as HourDoc,
  vespers:  hourVespers  as HourDoc,
  compline: hourCompline as HourDoc,
  midnight: hourMidnight as HourDoc,
};

const BLOOD_MAP: Record<string, BloodDoc> = {
  "section-a":  bloodA  as BloodDoc,
  "section-b1": bloodB1 as BloodDoc,
  "section-b2": bloodB2 as BloodDoc,
  "section-c":  bloodC  as BloodDoc,
  "section-d":  bloodD  as BloodDoc,
  "section-e":  bloodE  as BloodDoc,
};

export function loadHour(slug: string): HourDoc | null {
  return HOUR_MAP[slug] ?? null;
}

export function loadBlood(slug: string): BloodDoc | null {
  return BLOOD_MAP[slug] ?? null;
}

/** Returns the hour slug whose hour24 is the most recent (≤ current hour). */
export function getRecommendedHourSlug(currentHour: number): string {
  const sorted = [...hours].sort((a, b) => a.hour24 - b.hour24);
  // Default to the last prayer of the day (제12시, 18:00)
  let best = sorted[sorted.length - 1];
  for (const h of sorted) {
    if (h.hour24 <= currentHour) best = h;
  }
  // Before 6am — recommend midnight prayer
  if (currentHour < 6) {
    const midnight = sorted.find((h) => h.hour24 === 0);
    if (midnight) best = midnight;
  }
  return best.slug;
}
