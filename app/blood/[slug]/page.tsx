...
export default function BloodSlidesPage({ params }: { params: { slug: string } }) {
  const doc = loadBlood(params.slug);
  if (!doc || doc.placeholder) notFound();
  return (
    <PrayerSlides slides={doc.slides} docTitle={doc.title} docSubtitle={doc.subtitle}
      bookmarkKey={`bookmark:blood:${doc.slug}`} backHref="/blood" backLabel="보혈기도" />
  );
}
