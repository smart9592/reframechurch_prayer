    ...
    <ul className="space-y-3">
      {sections.map((s) => {
        if (s.placeholder) return (
          <li key={s.slug} className="block rounded-xl border border-dashed border-cream-300 bg-cream-50 p-4 opacity-60">
        ...
        return (
          <li key={s.slug}>
            <Link href={`/blood/${s.slug}`} className="block rounded-xl border border-cream-200 bg-white/60 p-4 shadow-sm transition active:scale-[0.99]">
              <div className="flex items-center justify-between">
